...
async function start() {
  ...
  const getInfo = await page.evaluate(() => {
    ...
    inputSelectValues.forEach((value) => {
      ...
      const carModelValues = Array.from(
      ...
      if (!brandModels[brandName]) {
        brandModels[brandName] = {
          brand: brandName,
          models: carModelValues,
        };
      } else {
        brandModels[brandName].models.push(...carModelValues);
      }
    });

    const cars = Object.values(brandModels);
